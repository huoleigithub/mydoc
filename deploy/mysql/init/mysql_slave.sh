...
# 设置主节点的信息
MYSQL_EXECUTE=$(MYSQL_PWD=${MYSQL_ROOT_PASSWORD} mysql -u root -e "CREATE USER '${MYSQL_REPLICATION_USER}'@'%' IDENTIFIED BY '${MYSQL_REPLICATION_PASSWORD}'; GRANT REPLICATION SLAVE ON *.* TO '${MYSQL_REPLICATION_USER}'@'%' IDENTIFIED BY '${MYSQL_REPLICATION_PASSWORD}'; CHANGE MASTER TO MASTER_HOST='172.25.0.201', MASTER_USER='${MYSQL_REPLICATION_USER}', MASTER_PASSWORD='${MYSQL_REPLICATION_PASSWORD}', MASTER_LOG_FILE='${MASTER_LOG_FILE}', MASTER_LOG_POS=${MASTER_LOG_POS}; START SLAVE;")
...
if [${MYSQL_EXECUTE_STATUE} = "ERROR"];then
  MYSQL_PWD=${MYSQL_ROOT_PASSWORD} mysql -u root -e "RESET SLAVE; CHANGE MASTER TO MASTER_HOST='172.25.0.201', MASTER_USER='${MYSQL_REPLICATION_USER}', MASTER_PASSWORD='${MYSQL_REPLICATION_PASSWORD}', MASTER_LOG_FILE='${MASTER_LOG_FILE}', MASTER_LOG_POS=${MASTER_LOG_POS}; START SLAVE;"
fi